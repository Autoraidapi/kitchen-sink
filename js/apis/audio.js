...
function displayProtocols() {
	try {
		...
		protocols = blackberry.audio.supportedProtocols(null);
...
function displayContentTypes() {
	try {
		var protocols, sb, i, contentTypes, j;
		...
		for (i = 0; i < protocols.length; i = i + 1) {
			...
			contentTypes = blackberry.audio.supportedContentTypes(protocols[i]);
			if (contentTypes) {
				sb.append("<ul>");
				for (j = 0; j < contentTypes.length; j = j + 1) {
